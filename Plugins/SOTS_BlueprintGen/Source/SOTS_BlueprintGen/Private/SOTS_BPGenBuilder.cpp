...
namespace
{
	...
	static UEdGraphNode* SpawnForLoopNode(UEdGraph* Graph, const FSOTS_BPGenGraphNode& NodeSpec)
	{
		...
		UK2Node_ForLoop* LoopNode = NewObject<UK2Node_ForLoop>(Graph);
		Graph->AddNode(LoopNode, /*bFromUI=*/false, /*bSelectNewNode=*/false);
		LoopNode->SetFlags(RF_Transactional);
		LoopNode->CreateNewGuid();
		LoopNode->PostPlacedNewNode();
		LoopNode->AllocateDefaultPins();

		LoopNode->NodePosX = static_cast<int32>(NodeSpec.NodePosition.X);
		LoopNode->NodePosY = static_cast<int32>(NodeSpec.NodePosition.Y);

		ApplyExtraPinDefaults(LoopNode, NodeSpec);
	...
	static int32 AddPinsFromSpec(UEdGraphNode* Node, EEdGraphPinDirection Direction, const TArray<FSOTS_BPGenPin>& PinDefs, TArray<FString>& OutWarnings)
	{
		...
		for (const FSOTS_BPGenPin& PinDef : PinDefs)
		{
			...
			UEdGraphPin* NewPin = Node->CreatePin(
				Direction,
				ResolvePinCategoryString(PinType.PinCategory),
				ResolvePinCategoryString(PinType.PinSubCategory),
			...
			NewPin->PinType.ContainerType = PinType.ContainerType;
			NewPin->PinType.PinCategory = PinType.PinCategory;
			NewPin->PinType.PinSubCategory = PinType.PinSubCategory;
			NewPin->PinType.PinSubCategoryObject = PinType.PinSubCategoryObject;
	...
	const FSOTS_BPGenGraphSpec& GraphSpec)
{
	...
	for (const FSOTS_BPGenGraphNode& NodeSpec : GraphSpec.Nodes)
	{
		...
		{
			NewNode = SpawnForLoopNode(FunctionGraph, NodeSpec);
		}
