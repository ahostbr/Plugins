...
static bool ResolveGraphSpecPathFromJobJson(
	const FString& JobFilePath,
	const TSharedPtr<FJsonObject>& JobRoot,
	FString& OutSpecPath,
	FString& OutFieldName)
{
	...
	static const FString GraphSpecFields[] = {
		FString(TEXT("GraphSpecFile")),
		FString(TEXT("GraphSpecPath"))
	};

	for (const FString& FieldName : GraphSpecFields)
	{
		FString Candidate;
		if (JobRoot->TryGetStringField(FieldName, Candidate) && !Candidate.IsEmpty())
		{
			FString CombinedPath = Candidate;
			if (FPaths::IsRelative(CombinedPath))
			{
				const FString JobDir = FPaths::GetPath(JobFilePath);
				CombinedPath = FPaths::Combine(JobDir, Candidate);
			}

			OutSpecPath = FPaths::ConvertRelativePathToFull(CombinedPath);
...
int32 USOTS_BPGenBuildCommandlet::Main(const FString& Params)
{
	...
	FString JobGraphSpecPath;
	FString JobGraphSpecField;
	ResolveGraphSpecPathFromJobJson(JobFilePath, JobRoot, JobGraphSpecPath, JobGraphSpecField);
	...
	FString ActiveGraphSpecPath;
	FString ActiveGraphSpecSource;

	if (!GraphSpecFilePath.IsEmpty())
	{
		ActiveGraphSpecPath = GraphSpecFilePath;
		ActiveGraphSpecSource = TEXT("command-line -GraphSpecFile");
	}
	else if (!JobGraphSpecPath.IsEmpty())
	{
		ActiveGraphSpecPath = JobGraphSpecPath;
		ActiveGraphSpecSource = JobGraphSpecField.IsEmpty()
			? TEXT("job JSON")
			: FString::Printf(TEXT("job JSON field '%s'"), *JobGraphSpecField);
	}

	if (!ActiveGraphSpecPath.IsEmpty())
	{
		UE_LOG(LogSOTS_BlueprintGen, Display,
			TEXT("SOTS_BPGenBuildCommandlet: Applying external GraphSpec file '%s' (%s)."),
			*ActiveGraphSpecPath,
			*ActiveGraphSpecSource);

		TSharedPtr<FJsonObject> GraphRoot;
		FString GraphError;
		if (!SOTS_BPGen_LoadJsonObject(ActiveGraphSpecPath, GraphRoot, GraphError))
		{
			UE_LOG(LogSOTS_BlueprintGen, Error,
				TEXT("SOTS_BPGenBuildCommandlet: %s"),
				*GraphError);
			return 1;
		}
	...
	{
		UE_LOG(LogSOTS_BlueprintGen, Display,
			TEXT("SOTS_BPGenBuildCommandlet: No external GraphSpec file provided; relying on inline GraphSpec if present."));
	}
